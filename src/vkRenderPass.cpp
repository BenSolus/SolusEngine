  ...
  : mRenderPass(VK_NULL_HANDLE), mDevice(device)
{
  ...
  {
    cc::throw_with_nested<std::runtime_error>("failed to create render "
                                              "pass!",
                                              PRETTY_FUNCTION_SIG);
  }
}

vk::eng::RenderPass::~RenderPass() noexcept
{
  destroyMembers();
}

void
vk::eng::RenderPass::destroyMembers()
{
