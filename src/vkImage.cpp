...
vk::eng::Image::Image() : Resource() {}

vk::eng::Image::Image(SharedPtrLogicalDevice const& device)
  : Resource(device)
{}

vk::eng::Image::~Image() noexcept { destroyMembers(); }

void
vk::eng::Image::createImage(uint32_t              width,
                            uint32_t              height,
                            VkFormat              format,
                            ...
                            VkMemoryPropertyFlags properties)
{
  ...
  if(vkCreateImage(vkDevice, &imageInfo, nullptr, &mResource) != VK_SUCCESS)
    cc::throw_with_nested<std::runtime_error>("failed to create image!",
                                              PRETTY_FUNCTION_SIG);
  ...
  if(vkAllocateMemory(vkDevice, &allocInfo, nullptr, &mDeviceMemory) != VK_SUCCESS)
    cc::throw_with_nested<std::runtime_error>("failed to allocate image memory!",
                                              PRETTY_FUNCTION_SIG);
...
void
vk::eng::Image::transitionImageLayout(VkCommandPool commandPool,
                                      VkFormat      format,
                                      ...
                                      VkImageLayout newLayout)
{
  ...
  else if((oldLayout == VK_IMAGE_LAYOUT_UNDEFINED) && (newLayout == VK_IMAGE_LAYOUT_DEPTH_STENCIL_ATTACHMENT_OPTIMAL))
  {
    ...
    cc::throw_with_nested<std::invalid_argument>("unsupported layout "
                                                   "transition!",
                                                 PRETTY_FUNCTION_SIG);
...
bool
vk::eng::Image::hasStencilComponent(VkFormat format)
{
